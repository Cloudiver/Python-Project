const askZJN5bWmgw = "amKNamUAspoO3SQT";
const asiIQ4WZn26y = "bwkDnVlAylbi2Yth";
const ackFF4mCcPZJ = "dV5dRk0AyvxwEd6q";
const aciZUF2YklYP = "fgVlvzTSTQPfwfLD";
const dskJwZJaQ8vX = "hUcmqyZgdJYpiniM";
const dsijgz4RUJqZ = "xyYWmPabZTt5CeUj";
const dckepxdVlwy0 = "ociQ0cAnsZ95Sej3";
const dci1yZjukmE2 = "px2CtiIPJ2Qwhzgk";
...
function dP4MNrTLaQw1vI6j9(data) {
    data = AES.decrypt(data, askZJN5bWmgw, asiIQ4WZn26y);
    data = DES.decrypt(data, dskJwZJaQ8vX, dsijgz4RUJqZ);
    data = BASE64.decrypt(data);
    return data
}
var pwkfkzaA7KV = (function () {
    function ObjectSort(obj) {
        var newObject = {};
        Object.keys(obj).sort().map(function (key) {
            newObject[key] = obj[key]
    ...
    return function (method, obj) {
        var appId = '771898a15a21040173d6f8eeedae95c9';
        var clienttype = 'WEB';
        ...
        param = AES.encrypt(param, ackFF4mCcPZJ, aciZUF2YklYP);
        return param
    }
})();

function s4LKor9mFGCTeTbSOc(method, object, callback, period) {
    const key = hex_md5(method + JSON.stringify(object));
    const data = getDataFromLocalStorage(key, period);
    if (!data) {
        var param = pwkfkzaA7KV(method, object);
        $.ajax({
...
// 自定义函数
// 对参数进行加密, 并返回结果
function getPostParamCode(method, city){
    let object = {};
    object['city'] = city
    return pwkfkzaA7KV(method, object)  // 加密函数, 下次执行时需要手动修改函数名
}
